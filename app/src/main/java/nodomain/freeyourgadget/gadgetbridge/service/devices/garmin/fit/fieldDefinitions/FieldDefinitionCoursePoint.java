...
public class FieldDefinitionCoursePoint extends FieldDefinition {
    ...
    @Nullable
    public static CoursePoint fromId(final int id) {
        for (final CoursePoint candidate : CoursePoint.values()) {
            if (id == candidate.id) {
                return candidate;
            }
        }
    ...
    @Nullable
    @Override
    public CoursePoint decode(ByteBuffer byteBuffer) {
    ...
    public void encode(ByteBuffer byteBuffer, Object o) {
        if (o instanceof CoursePoint coursePoint) {
            baseType.encode(byteBuffer, coursePoint.id, scale, offset);
            return;
        }
